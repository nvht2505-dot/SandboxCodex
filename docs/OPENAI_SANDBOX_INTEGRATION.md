...
## Implementation scaffold trong repository

Repository hiện có module `src/openclaw.js` để chuẩn hóa các primitive đầu tiên của OpenClaw:

- `buildOpenClawSystemPrompt()` tạo system instruction thống nhất cho agent.
- `createOpenClawPlan()` tạo plan có schema ổn định từ intent của user.
- `createToolCallPolicy()` định nghĩa tool allowlist, command blocklist, timeout và giới hạn vòng tự sửa lỗi.
- `validateWorkflowEvent()` kiểm tra event runtime trước khi stream về UI hoặc lưu audit log.
- `redactSecrets()` che API key/token/secret khỏi log và output test.

Module này chưa gọi OpenAI API trực tiếp; nó là lớp contract an toàn để backend Orchestrator dùng khi thêm client OpenAI, streaming Responses API và Tool Gateway thật.
